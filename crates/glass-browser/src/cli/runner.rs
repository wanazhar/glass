...
use super::args::{
    AgentCommand, AgentHarness, BackendCommand, CertifyCommand, CheckpointCommand, Cli, Commands,
    DaemonCommand, ExperimentCommand, IrCommand, KnowledgeCommand, KnowledgeInvalidationState,
    McpClient, MemoryCommand, NeovimCommand, ProfileCommand, ProjectCommand, ProjectGraphCommand,
    ProjectProcessCommand, ReplayCommand, ResultCommand, SnapshotCommand, SurfaceCommand,
    TaskCommand, WorkflowAuthoringCommand, WorkspaceCommand,
};
...
use crate::development::{
    Actor, ExperimentManager, HarnessRequest, LinkProvenance, ProcessSnapshot, ProjectWorkspace,
...
pub async fn dispatch(cli: Cli) -> BrowserResult<()> {
    dispatch_product(cli, true).await
}
...
async fn dispatch_product(cli: Cli, development_enabled: bool) -> BrowserResult<()> {
    ...
    match &cli.command {
        ...
        Some(Commands::Tui) | None if cli.prompt.is_none() => {
            if should_run_tui(
                ...
                return crate::tui::app::run_tui_for_product(&cli, development_enabled).await;
            }
            print_start_here();
            return Ok(());
        }
