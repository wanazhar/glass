...
version="$(cargo metadata --no-deps --format-version 1 | python3 -c 'import json,sys; print(next(p["version"] for p in json.load(sys.stdin)["packages"] if p["name"]=="glass"))')"
...
fuzz_status=passed
if ! cargo check --manifest-path fuzz/Cargo.toml --locked --all-targets >/dev/null; then fuzz_status=failed; fi
...
check_commands = {
    ...
    "audit": "cargo audit",
    "fuzz-check": "cargo check --manifest-path fuzz/Cargo.toml --locked --all-targets",
}
passed_checks = [
