...
def main() -> None:
    ...
    if baseline.get("version") != "0.2.6":
        fail("release baseline must be 0.2.6")
    if baseline.get("tag") != "v0.2.6":
        fail("release baseline tag must be v0.2.6")
    ...
    if matrix.get("next_release") != "0.2.7":
        fail("next_release must be 0.2.7")
    ...
    required_text = {
        "README.md": "| 0.2.6 | Current published release |",
        "CHANGELOG.md": "## [Unreleased] — 0.2.7",
        "docs/release-checklist.md": "next crates.io release, `0.2.7`",
        "docs/plan/analysis/release-audit-028.md": "`0.2.6 published; source-only GitHub Release",
    }
