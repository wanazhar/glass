...
def main() -> None:
    ...
    if baseline.get("version") != "0.2.4":
        fail("release baseline must be 0.2.4")
    if baseline.get("tag") != "v0.2.4":
        fail("release baseline tag must be v0.2.4")
    ...
    if matrix.get("next_release") != "0.2.5":
        fail("next_release must be 0.2.5")
