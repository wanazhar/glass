...
| `glass.editor` | `glass.editor.buffers`, `glass.editor.checkpoint.create`, `glass.editor.checkpoint.restore`, `glass.editor.checkpoints`, `glass.editor.comment.add`, `glass.editor.comment.resolve`, `glass.editor.comments`, `glass.editor.diff`, `glass.editor.fim`, `glass.editor.open`, `glass.editor.proposal.accept`, `glass.editor.proposal.accept_pack`, `glass.editor.proposal.create`, `glass.editor.proposal.reject`, `glass.editor.proposals`, `glass.editor.replace`, `glass.editor.replace_selection`, `glass.editor.save`, `glass.editor.selection` | Attributed editor buffers, live selections, review comments/proposals, checkpoints, fill-in-the-middle ghosts, conflict-safe replacements, and saves. |
| `glass.eval` | `glass.eval.cancel`, `glass.eval.execute`, `glass.eval.list`, `glass.eval.reset`, `glass.eval.start`, `glass.eval.stop` | Persistent language kernels, governed Glass bindings, and bounded execution state. |
| `glass.experiment` | `glass.experiment.collect`, `glass.experiment.compare`, `glass.experiment.create`, `glass.experiment.list`, `glass.experiment.remove`, `glass.experiment.select` | Isolated worktrees, automatic measured evidence, deterministic comparison, selection, and cleanup. |
...
| `glass.git` | `glass.git.blame`, `glass.git.branch.create`, `glass.git.branch.switch`, `glass.git.branches`, `glass.git.commit`, `glass.git.conflicts`, `glass.git.diff`, `glass.git.discard`, `glass.git.fetch`, `glass.git.merge`, `glass.git.pull`, `glass.git.push`, `glass.git.rebase`, `glass.git.stage`, `glass.git.stash.list`, `glass.git.stash.pop`, `glass.git.stash.push`, `glass.git.status`, `glass.git.unstage`, `glass.git.worktree.create`, `glass.git.worktree.list`, `glass.git.worktree.remove` | Native repository inspection, branches, staging, commits, fetch/pull/merge/rebase, confirmed discard/push, stashes, conflicts, and worktrees. |
| `glass.github` | `glass.github.review`, `glass.github.ship` | `review` reads the current branch pull request and checks through the authenticated `gh` CLI under resident workspace actor attribution; no confirmation is required. `ship` creates a pull request from a named feature branch through authenticated `gh`; trusted-workspace mutation authority and explicit per-call confirmation are required. |
| `glass.graph` | `glass.graph.explain`, `glass.graph.link`, `glass.graph.path`, `glass.graph.query`, `glass.graph.source` | Revisioned causal graph observations, typed evidence links, paths, source links, and explanations. |
| `glass.lsp` | `glass.lsp.code_actions`, `glass.lsp.completion`, `glass.lsp.declaration`, `glass.lsp.definition`, `glass.lsp.diagnostics`, `glass.lsp.document_symbols`, `glass.lsp.events`, `glass.lsp.formatting`, `glass.lsp.hover`, `glass.lsp.implementation`, `glass.lsp.inlay_hints`, `glass.lsp.list`, `glass.lsp.range_formatting`, `glass.lsp.raw`, `glass.lsp.references`, `glass.lsp.rename`, `glass.lsp.semantic_tokens`, `glass.lsp.signature_help`, `glass.lsp.start`, `glass.lsp.stop`, `glass.lsp.workspace_symbols` | Shared LSP lifecycle and the complete typed language-operation surface. |
...
| `glass.neovim` | `glass.neovim.probe`, `glass.neovim.start` | Native Neovim PTY and RPC capability probing and lifecycle. |
| `glass.process` | `glass.process.health`, `glass.process.input`, `glass.process.list`, `glass.process.logs`, `glass.process.ports`, `glass.process.remove`, `glass.process.resize`, `glass.process.restart`, `glass.process.start`, `glass.process.stop` | Resident PTY lifecycle, I/O, health, ports, and bounded logs. |
...
| `glass.project` | `glass.project.attach` | External actor attachment and declared project authority. |
| `glass.semantic` | `glass.semantic.diff`, `glass.semantic.inspect`, `glass.semantic.links` | Browser observation diffs plus source/runtime graph inspection and links. `glass.semantic.inspect` requires an `entity` argument; use `glass.browser.semantic` for current page semantics. |
| `glass.task` | `glass.task.cancel`, `glass.task.create`, `glass.task.crew`, `glass.task.evidence`, `glass.task.get`, `glass.task.inspect`, `glass.task.list`, `glass.task.override-blocked`, `glass.task.pause`, `glass.task.plan`, `glass.task.reassign`, `glass.task.resume`, `glass.task.retry`, `glass.task.verify`, `glass.task.wake` | Value-free task compilation plus autonomous verified DAG lifecycle, overnight crew wakes, and evidence. `inspect` and `verify` are stable external-agent names; `get` and `evidence` remain compatible aliases. The Agent checklist uses `glass.todo.*`, not this family. |
| `glass.todo` | `glass.todo.complete`, `glass.todo.list`, `glass.todo.write` | Workspace-local Agent checklist persisted at `.glass/todos/session.json`. Keep at most one `active` item. This is not the overnight task DAG. |
