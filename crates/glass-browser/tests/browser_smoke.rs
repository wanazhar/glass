...
use glass_browser::reliability_runner::{ReliabilityRunOptions, run_reliability_scenario};
use glass_browser::{
...
async fn browser_session_drives_a_local_fixture() {
    ...
    let workflow = WorkflowDefinition {
        ...
        outputs: BTreeMap::from([(
            ...
            WorkflowOutputDeclaration {
                value_type: glass_browser::browser::session::WorkflowValueType::String,
    ...
    assert!(
        context.incomplete.contains(
            &glass_browser::browser::session::ObservationIncompleteReason::ShadowBoundary
        )
    );

    let clip_capture = session
        .capture_visual(&glass_browser::browser::session::VisualCaptureOptions {
            clip: Some(glass_browser::browser::session::VisualClip {
                x: 0.0,
                y: 0.0,
                width: 120.0,
                height: 80.0,
    ...
    let jpeg_capture = session
        .capture_visual(&glass_browser::browser::session::VisualCaptureOptions {
            format: glass_browser::browser::session::VisualFormat::Jpeg,
            quality: Some(80),
            clip: Some(glass_browser::browser::session::VisualClip {
                x: 10.0,
                y: 10.0,
    ...
    let webp_capture = session
        .capture_visual(&glass_browser::browser::session::VisualCaptureOptions {
            format: glass_browser::browser::session::VisualFormat::Webp,
            quality: Some(80),
            clip: Some(glass_browser::browser::session::VisualClip {
                x: 10.0,
                y: 10.0,
    ...
    let mut screencast = session
        .start_screencast(
            glass_browser::browser::session::VisualFormat::Jpeg,
            70,
            640,
            480,
        )
    ...
    assert!(
        context
            .incomplete
            .contains(&glass_browser::browser::session::ObservationIncompleteReason::FrameBoundary)
    );
    assert!(
        context
            .incomplete
            .contains(&glass_browser::browser::session::ObservationIncompleteReason::Canvas)
    );
    ...
    assert!(
        raced
            .incomplete
            .contains(&glass_browser::browser::session::ObservationIncompleteReason::MutationRace)
    );
    session
        .evaluate(
    ...
    assert!(
        huge.incomplete
            .contains(&glass_browser::browser::session::ObservationIncompleteReason::VisibleText)
    );
    assert!(huge.text.len() <= glass_browser::browser::session::COMPACT_TEXT_MAX_BYTES);
...
fn write_reliability_evidence(
    scenario_id: &str,
    evidence: &glass_browser::reliability_runner::ReliabilityRunEvidence,
) {
