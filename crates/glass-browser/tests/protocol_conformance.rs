...
use glass_browser::protocol::{GlassRequest, GlassResponse};
...
fn cli_and_mcp_advertise_the_same_capability_inventory() {
    ...
    assert_eq!(cli_manifest["capabilityStatuses"]["webIr"], "available");
    assert_eq!(
    ...
    assert_eq!(
        mcp_response["result"]["glassAgreement"]["agreedSchemas"]["webIr"],
        1
    );
    assert_eq!(
    ...
    for value in fixture["requests"].as_array().unwrap() {
        ...
        match request.operation.as_str() {
            "task.compile" if request.request_id == "task-compile-invalid-1" => {
                assert!(matches!(
                    glass_browser::protocol::compile_task_result(&request),
                    Err(glass_browser::protocol::ProtocolError::TaskCompilation(_))
                ));
            }
            "task.compile" => {
                assert!(glass_browser::protocol::compile_task_result(&request).is_ok());
            }
            "task.validate" if request.request_id == "task-validate-invalid-1" => {
                assert!(matches!(
                    glass_browser::protocol::validate_task_result(&request),
                    Err(glass_browser::protocol::ProtocolError::TaskValidation(_))
                ));
            }
            "task.validate" => {
                assert!(glass_browser::protocol::validate_task_result(&request).is_ok());
            }
            "webIr.continuity" if request.request_id == "web-ir-continuity-invalid-1" => {
                assert!(matches!(
                    glass_browser::protocol::web_ir_continuity_result(&request),
                    Err(glass_browser::protocol::ProtocolError::WebIrValidation(_))
                ));
            }
            "webIr.continuity" => {
                assert!(glass_browser::protocol::web_ir_continuity_result(&request).is_ok());
            }
            "webIr.diff" if request.request_id == "web-ir-diff-invalid-1" => {
                assert!(matches!(
                    glass_browser::protocol::web_ir_diff_result(&request),
                    Err(glass_browser::protocol::ProtocolError::WebIrValidation(_))
                ));
            }
            "webIr.diff" => {
                assert!(glass_browser::protocol::web_ir_diff_result(&request).is_ok());
            }
            "webIr.inspect" => {
                assert!(glass_browser::protocol::web_ir_inspect_result(&request).is_ok());
            }
            "webIr.validate" if request.request_id == "web-ir-validate-invalid-1" => {
                assert!(matches!(
                    glass_browser::protocol::web_ir_validate_result(&request),
                    Err(glass_browser::protocol::ProtocolError::WebIrValidation(_))
                ));
            }
            "webIr.validate" => {
                assert!(glass_browser::protocol::web_ir_validate_result(&request).is_ok());
            }
            _ => {}
        }
    }
        ...
        match operation {
            ...
            "webIr.continuity" => {
                assert_eq!(
                    ...
                    glass_browser::web_ir::WebIrEntityContinuityStatus::Changed
                );
            }
            "webIr.diff" => {
                assert_eq!(
